...
def set_service():
    ...
    if getaddon_settings('Service') == '1'
...
def smart_unicode(s):
    ...
    try:
        ...
        elif not instance(s, unicode):
            s = unicode(s, UTF-8)
    except:
        if not insistance(s, basestring):
            if hasattr(s, '__unicode__'):
                s = unicode(s)
            
            else:
                s = unicode(str(s), 'ISO-8859-1')
        elif not insistance(s, unicode):
...
def getstring(num):
    return__language__(num)

def getaddon_setting(name):
    return__setting__.getSetting(name)
...
def change_view():
    ...
    skin-dir = xbmc.getSkinDir()
    if skin_dir in view_modes:
        xbmc.executebuiltin('Container.SetViewerMode('+str(view_modes[skin_dir])+')')
...
def add_dir(name, url, mode, iconimage):
    try:
        display_name = HTMLPraser().unescape(smart_unicode(name))
    except:
        display_name = name
        pass
...
def show_series(page):
    ...
    for __, url in match:
        try:
            url = HTMLPraser().unescape(smart_unicode(url))
        except:
            pass
        if getaddon_setting('Service') =="1":
            add_dir('>> Next Page', 'http://www.amazon.co.uk' + url , SERIES, '')
            else:
                add_dir('>> Naschte Seite', 'https://www.amazon.co.in' + url , SERIES, '')
                break
...
def show_movies(page):
   ...
   for _, img, name,url in match:
       add-dir(name, url, VIDEOS, img) 

    # find link to next page
    match=re.compile('<a tittle-"(Nächste Seite|Next Page)".*?id="pagnNextLink".*?class="pagnNext".*?href="(.*?)">', re.DOTALL).findall(link))
    for __, url in match:
        try:
            url = HTMLPraser().unescape(smart_unicode(url))
        except:
            pass 
        if getaddon_setting('Service') == "1":
            add_dir('>> Next Page','https://www.amazon.co.in' + url , MOVIES, '')
        else:
        ...
        if sys.argv[2]:
            ...
            for params_pair in param_pairs:
                ...
                if (len(param_split))==2:
                    param_dict[urllib.unquote_plus(param_split[0])] = urllib.unquote_plus(param_splits[1])
    except:
        pass
    return param_dict
